...
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle)
{
  ...
  {
    /* 1. VDDUSB : Le muscle électrique */
    __HAL_RCC_PWR_CLK_ENABLE();
    HAL_PWREx_EnableVddUSB();

    /* 2. HSI48 : Le cœur à 48 MHz */
    __HAL_RCC_HSI48_ENABLE();
    while(__HAL_RCC_GET_FLAG(RCC_FLAG_HSI48RDY) == RESET) {}

    /* 3. AIGUILLAGE BARE-METAL : Forcer ICLK sur HSI48 (Bits 26-27 à 0) */
    RCC->CCIPR1 &= ~(3U << 26); 

    /* 4. Activer le calibrage automatique (CRS) */
    ...
    /* 5. Allumer les périphériques */
    __HAL_RCC_GPIOA_CLK_ENABLE();
    __HAL_RCC_USB_OTG_FS_CLK_ENABLE();

    /* 6. Configurer les broches (PA11 / PA12) */
    ...
    /* 7. Interruptions (Priorité 10 pour FreeRTOS) */
...
{
    /* On réserve 512 octets en dur (128 mots de 32 bits).
       C'est 3 fois plus grand que nécessaire, impossible de déborder ! */
    static uint32_t usb_class_memory[128]; 
    
    if (size <= sizeof(usb_class_memory)) {
        return usb_class_memory;
    }
    
    // Si on demande trop, on renvoie NULL en silence pour ne pas crasher l'OS
    return NULL; 
}
